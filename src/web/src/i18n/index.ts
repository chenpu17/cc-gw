        ...
        sections: {
          configuration: {
            ...
            items: [
              ...
              '在 IDE 中配置接入：Claude Code（含 VS Code 插件）统一设置 Base URL=http://127.0.0.1:4100/anthropic/v1，客户端会自动附加 /v1/messages，cc-gw 已兼容；Codex 指向 http://127.0.0.1:4100/openai/v1，并使用 cc-gw 生成的 API Key。'
            ]
          },
          usage: {
            title: '二、日常使用',
        ...
        sections: {
          configuration: {
            ...
            items: [
              ...
              'Point your IDEs to the gateway: both the Claude CLI and the VS Code extension can use `http://127.0.0.1:4100/anthropic/v1` (clients append `/v1/messages`, which cc-gw tolerates), while Codex should target `http://127.0.0.1:4100/openai/v1`; authenticate with a cc-gw API key.'
            ]
          },
          usage: {
            title: '2. Daily usage',
