        ...
        sections: {
          configuration: {
            ...
            items: [
              ...
              '在 IDE 中配置接入：Claude Code CLI 设置 ANTHROPIC_BASE_URL=http://127.0.0.1:4100，VS Code 插件自定义 Base URL 填写 http://127.0.0.1:4100/anthropic（插件会自动补 `/v1/messages`），Codex 指向 http://127.0.0.1:4100/openai/v1，并使用 cc-gw 生成的 API Key。'
            ]
          },
          usage: {
            title: '二、日常使用',
        ...
        sections: {
          configuration: {
            ...
            items: [
              ...
              'Point your IDEs to the gateway: set `ANTHROPIC_BASE_URL=http://127.0.0.1:4100` for the Claude CLI, use `http://127.0.0.1:4100/anthropic` in the Claude Code VS Code extension (it appends `/v1/messages`), and target `http://127.0.0.1:4100/openai/v1` for Codex—authenticate with a cc-gw API key.'
            ]
          },
          usage: {
            title: '2. Daily usage',
